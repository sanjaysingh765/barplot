...
#                     USEs : Rscript script_name data_file plot_title figure_name.png  significant_placement y_axis_limit
...
ggbarplot(select, x = "Var2", y = "value",
  ...
  #stat_compare_means(label.y = c(160,270))+      # Add global p-value
  stat_compare_means(label = "p.signif", #"p.signif"  "p.format"
                     method = "t.test",
                     label.y = c(as.numeric(args[4]),as.numeric(args[5])), #change argumnet number here
                     ref.group = "Control")+  # Pairwise comparison against reference
scale_y_continuous(expand=c(0,0), limits = c(0, as.numeric(args[6])))+
  theme(
